...
def selenium(pytestconfig):
    browser_name = pytestconfig.getini(
        "browser_name"
    )  # Получаем переменную browser_name
    ...
    if pytestconfig.getini("browser_name") == "chrome":
        ...
        if (
            pytestconfig.getini("headless") == "True" and browser_name == "chrome"
        ):  # Условия отображения интерфейса
