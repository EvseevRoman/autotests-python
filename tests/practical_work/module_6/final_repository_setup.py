...
class TestValidation:
    ...
    def test_validation_search_multiple_input_fields(self, selenium):
        ...
        with allure.step('Дождаться того, что элемент поля для выбора языка присутствуют в DOM страницы и виден'):
        ...
        with allure.step('Ввести в поле с названием файла текст "environment.yml"'):
            action_chains.send_keys_to_element(field_filename, "environment.yml").perform()
    ...
    def test_validation_slider_checkbox_radiobutton(self, selenium):
        ...
        with allure.step('Сделать скролл страницы до слайдера "Длительность",'
        ...
        with allure.step("Зажать левую кнопку мыши на правом ползунке"
    ...
    def test_validation_chart(self, selenium):
        ...
        with allure.step('Открыть страницу https://github.com/microsoft/vscode/graphs/commit-activity'):
            selenium.get("https://github.com/microsoft/vscode/graphs/commit-activity")
        with allure.step('Развернуть окно браузера на весь экран'):
            selenium.maximize_window()
        with allure.step('Дождаться наличия искомого элемента графика в DOM-дереве страницы'):
        ...
        with allure.step('Переместить курсор мыши на искомый элемент графика'):
            action_chains.move_to_element(elem_chart)\
                .perform()
        with allure.step('Дождаться наличия тултипа искомого элемента графика в DOM-дереве страницы'):
        ...
        with allure.step('Проверка на соответствии тултипа ожидаемому значению ("138 commits the week of Oct 6")'):
