...
class TestValidation:
    ...
    def test_validation_search(self, go_to_url, page: Page):
        ...
        with allure.step(f'Ввести в поле поиска, сортировку по ключевому слову {search_text}'):
        ...
        with allure.step(f'Выполнение проверки на наличие ключевого слова {search_text} в заголовках'):
            check_results_search(page, path, search_text)

    @allure.title('Проверка фильтрации страницы issues на сайте github по выбранному значению из выпадающего списка')
    def test_validation_search_drop_down(self, go_to_url, page: Page):
        ...
        with allure.step('Сделать клик по полю поиска в выпадающем списке'):
            page.locator("//div[@id='__primerPortalRoot__']/div/div/div/div[2]/div/span/input").click()
        with allure.step('Ввести текст "bpasero" в поле поиска'):
            page.locator("//div[@id='__primerPortalRoot__']/div/div/div/div[2]/div/span/input").type(search_text)
        with allure.step('Найти искомого автора в обновлённом выпадающем списке и сделать по нему клик'):
    ...
    def test_validation_search_multiple_input_fields(self, go_to_url, page: Page):
        ...
        with allure.step(f"Выбрать язык {language} в поле языка, на котором написан код"):
        ...
        with allure.step(f"Выполнить проверку того, что найденные результаты написаны на языке {language}"):
            check_results_search(page, path_language, language)
        with allure.step(f"Выполнить проверку того, что найденные результаты c количеством звёзд {count_stars}"):
    ...
    def test_validation_slider_checkbox_radiobutton(self, go_to_url, page: Page):
        ...
        with allure.step(f"Выберать радиобаттон с названием {type_training} в разделе «Тип обучения на платформе»"
                         " в DOM-дереве страницы"):
            page.locator(f"//label/child::span[contains(text(), '{type_training}')]").click()
        ...
        with (allure.step("Указать в поле «Длительность» диапазон до 12 месяцев")):
            right_slider = page.locator("(//button[@aria-label='Изменить диапозон'])[2]")
        ...
        with allure.step('Найти кнопку раскрывающегося списка Тематик'):
            page.locator('//button[@class="ui-expand-button filter-checkboxes__button ui-expand-button--small"]')\
                .click()
        with allure.step(f"Выбрать в Тематике чекбокс {topic}"):
            page.locator(f"//span/child::span[contains(text(), '{topic}')]").click()

        with allure.step('Выполнить проверку того, что карточки курсов содержат выбранные параметры фильтров'):
    ...
    def test_validation_chart(self, go_to_url, page: Page):
        ...
        with allure.step('Переместить курсор мыши на искомый элемент графика'):
            page.wait_for_selector("//*[contains(@class, 'viz')]//*[contains(@class, 'bar')][27]").hover()

        with allure.step('Проверка тултипа на соответствии ожидаемому значению ("238 commits the week of Nov 3")'):
            assert page.locator("//div[@class='svg-tip n']").inner_text() == '238 commits the week of Nov 3'
