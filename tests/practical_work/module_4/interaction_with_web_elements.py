...
class TestInteraction:
    def test_field_search(self, selenium):
        ...
        field_search = wait.until(
            EC.visibility_of_element_located((By.ID, "repository-input"))
        )
        field_search.click()
        text = "in:title bug"
    ...
    def test_multiple_input_fields(self, selenium):
        ...
        action_chains.click_and_hold(check_select).pause(1).perform()
    ...
    def test_chart(self, selenium):
        ...
        action_chains = ActionChains(selenium)
        action_chains.move_to_element(chart).perform()
        elem_chart = selenium.find_element(
            By.XPATH, "//*[contains(@class, 'viz')]//*[contains(@class, 'bar')][28]"
        )
