...
class TestPlacingOrderAuthorizedUser:
    ...
    def test_displaying_button_proceed_payment(self, open_page, selenium):
        ...
        with step("Проверка авторизации пользователя"):
            ...
            if button_authoriz == "Войти":
                authorization_user(
                    wait, action_chains, go_to_element, username, password
                )
    ...
    @allure.title(
        'Ввод данных в форме "Оформление заказа", авторизованным пользователем'
    )
    ...
    @allure.title(
        "Ввод даты оформления доставки на завтра, авторизованным пользователем"
    )
    def test_entering_delivery_date(self, open_page, selenium):
        ...
        with step(
            "Убедиться. что поле принимает и корректно отображает введённую дату"
        ):
            allure.attach(
                selenium.get_screenshot_as_png(),
                name="Дата доставки",
    ...
    def test_order_confirmation(self, open_page, selenium):
        ...
        with step(
            "Убедиться, что открылась страница с подтверждением полученного заказа"
        ):
            assert wait.until(EC.url_contains("order-received"))
            pass

    @allure.title(
        "Проверка корректности введённых данных и суммы заказа при оформлении заказа"
    )
    def test_checking_correctness_data_and_order(self, open_page, selenium):
        ...
        with step('Ввести в поле "Телефон" валидный номер телефона'):
            field_telephone.clear()
            field_telephone.send_keys(value_telephone)
            value_email = wait_visible_element(wait, path_field_email).get_attribute(
                "value"
            )
        with step('Навести курсор на поле "Дата заказа"'):
            calendar = go_to_element(wait, action_chains, path_calendar)
        ...
        with step(
            "Проверить корректность введённых данных и суммы заказа на странице подтверждения заказа"
        ):
            expected_address = wait_visible_element(
                wait, path_expected_address
            ).text.split("\n")
            ...
            expected_amount_order = wait_visible_element(
                wait, path_expected_amount_order
            ).text.replace(",00", "")
            expected_method_payment = wait_visible_element(
                wait, path_expected_method_payment
            ).text
            ...
            ), "Дата в заказе не соответствует введённой"
            pass
