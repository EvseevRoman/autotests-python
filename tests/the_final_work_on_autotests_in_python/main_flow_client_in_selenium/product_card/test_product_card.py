...
class TestProductCard:
    ...
    def test_pizza_names_slider_and_product_card(self, open_page, selenium):
        ...
        with step('Запомнить название пиццы и сделать клик по картинке товара'):
            title_pizza_in_slider = wait_visible_element(wait, path_title_pizza_in_slider).text.lower()
            action_chains.click().perform()
        with step('Сравнение название пиццы в карточке товара с названием на картинке в слайдере'):
            title_pizza_in_card = wait_visible_element(wait, path_title_pizza_in_card).text.lower()
    ...
    def test_availability_of_additional_options(self, open_page, selenium):
        ...
        with step('Проверка наличие сырного и колбасного борта для пиццы'):
            ...
            options = wait.until(EC.visibility_of_all_elements_located((By.XPATH, path_option)))
