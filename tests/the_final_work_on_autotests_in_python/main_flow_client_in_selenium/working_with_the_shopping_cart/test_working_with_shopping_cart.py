...
class TestWorkShoppingCart:
    ...
    def test_contents_shopping_cart_after_adding_items(self, open_page, selenium):
        ...
        path_title_second_pizza_in_slider = "(//li[contains(@class, 'slick-active')]//h3)[2]"
        ...
        with step('Навести курсор любую понравившуюся картинку с пиццей'):
            title_first_pizza_in_slider = wait_visible_element(wait, path_title_first_pizza_in_slider).text.lower()
        ...
        with step('Сделать клик по борту для пиццы "Сырный"'):
            ...
            count_first_pizza_in_card = wait_visible_element(wait, path_count_first_pizza_in_cart)\
                .get_attribute("value")
        ...
        with step('Сделать клик по вкладке "Главная страница"'):
            action_chains.click().perform()
        with step('Навести курсор любую понравившуюся картинку с пиццей'):
            title_second_pizza_in_slider = wait_visible_element(wait, path_title_second_pizza_in_slider).text.lower()
        ...
        with step('Сделать клик по борту для пиццы "Колбасный"'):
            ...
            selenium.execute_script('document.activeElement.blur()')
            count_second_pizza_in_card = wait_visible_element(wait, path_count_first_pizza_in_cart)\
                .get_attribute("value")
        with step('Навести курсор на кнопку "В корзину"'):
            go_to_element(wait, action_chains, path_button)
        with step('Сделать клик по кнопке "В корзину"'):
        ...
        with step('Сделать клик по иконке корзины'):
            action_chains.click().perform()
        with step('Проверка того, что в корзине все добавленные пиццы, в нужном количестве и опциями.'):
            title_first_pizza_in_cart = wait_visible_element(wait, path_title_first_pizza_in_cart).text.lower()
            title_second_pizza_in_cart = wait_visible_element(wait, path_title_second_pizza_in_cart).text.lower()
            count_first_pizza_in_cart = wait_visible_element(wait, path_count_first_pizza_in_cart)\
                .get_attribute("value")
            count_second_pizza_in_cart = wait_visible_element(wait, path_count_second_pizza_in_cart)\
                .get_attribute("value")
            option_first_pizza_in_cart = wait_visible_element(wait, path_option_first_pizza_in_cart).text
            option_second_pizza_in_cart = wait_visible_element(wait, path_option_second_pizza_in_cart).text
            assert title_first_pizza_in_slider.replace('«', '"').replace('»', '"') == title_first_pizza_in_cart\
                and count_first_pizza_in_card == count_first_pizza_in_cart\
                and "Сырный" in option_first_pizza_in_cart\
                and title_second_pizza_in_slider.replace('«', '"').replace('»', '"') == title_second_pizza_in_cart\
    ...
    def test_changing_number_pizzas_in_basket(self, open_page, selenium):
        ...
        path_button_basket = "//li[contains(@class, 'slick-active')]//a[contains(text(), 'В корзину')]"
        ...
        with step('Навести курсор на кнопку "Обновить корзину"'):
            button_update_cart = wait.until(EC.element_to_be_clickable((By.XPATH, path_button_update_cart)))
